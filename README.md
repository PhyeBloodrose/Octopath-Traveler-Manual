# How to Play this Manual
This manual is intended to play on a freshly started playthrough.

A random protagonist will be selected for you to play through their chapter 1 and to see where you spawn in the world!

You'll have to gather map pieces or maps to leave your starting town, and to enter any other settlement you require their settlement passes!

These are randomized by default: Weapon Types, Element Types, Jobs, Travelers, Travel Chapters, Settlement Passes and Maps.

## Goal Options
- Complete anyone's Chapter 4
- Defeat Galdera

You can enable as little or as many goals as you wish. The goal button will only be able to be checked whenever you have achieved ALL victory conditions.

## Optional Yaml settings
- Chestsanity - Adds all of the chests as extra locations.
- Expanded Story - Adds each Journal entry for travelers as a check.
- Side Stories - Adds all the side stories as check.
- Hidden Items - Any Hidden Items you can find from Scrutinizing and Inquiring will be added as a check. Except for those which are missable.
- Dungeons - Adds all of the Chests, Bosses and Side Stories inside of optional Dungeons as checks.
- High Level - If enabled will allow for locations that are above level 50 to be available if possible.
- Randomized Job Skills - Will add the Job Skills into the pool when enabled and as checks to be unlocked. 
- Randomized Job Support Skills - Will add the Job Support Skills into the pool when enabled and as checks to be unlocked.
- Randomized Items - If enabled, one of each item will be added into the multiworld. Do as you wish with them, restrict yourself from using them until you own them or use something that can generate it into your game!
- Progressive Lands - If enabled you'll unlock the various lands' tiers progressively 1 to 3. If disabled each Tier has its own Map Piece.
- Solo Run - Only ever have one Traveler while doing their Chapters. This option only works for goals which doesn't require you to defeat Galdera.
- Death Link - When you receive a death link, just reset the game. A game over is always followed by start from last save point anyway! Whenever you game over, you send a death link in the client.